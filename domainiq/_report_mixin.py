...
class _ReportMixin(_SyncRequestable):
    def domain_report(self, domain: str) -> DomainReport:
        ...
        return parse_domain_report(self._make_json_request(build_domain_report_params(domain)))

    def name_report(self, name: str) -> NameReportResult:
        """Get registrant name report."""
        return cast("NameReportResult", self._make_json_request(build_name_report_params(name)))
    ...
    def email_report(self, email: str) -> EmailReportResult:
        ...
        return cast("EmailReportResult", self._make_json_request(build_email_report_params(email)))

    def ip_report(self, ip: str) -> IpReportResult:
        """Get IP address summary report."""
        return parse_ip_report_result(self._make_json_request(build_ip_report_params(ip)))
...
class _AsyncReportMixin(_AsyncRequestable):
    async def domain_report(self, domain: str) -> DomainReport:
        ...
        return parse_domain_report(await self._make_json_request(build_domain_report_params(domain)))

    async def name_report(self, name: str) -> NameReportResult:
        """Get registrant name report asynchronously."""

        return cast("NameReportResult", await self._make_json_request(build_name_report_params(name)))
    ...
    async def organization_report(self, organization: str) -> OrganizationReportResult:
        ...
        return cast(
            "OrganizationReportResult",
            await self._make_json_request(build_organization_report_params(organization)),
        )

    async def email_report(self, email: str) -> EmailReportResult:
        """Get registrant email report asynchronously."""

        return cast("EmailReportResult", await self._make_json_request(build_email_report_params(email)))

    async def ip_report(self, ip: str) -> IpReportResult:
        """Get IP address summary report asynchronously."""

        return parse_ip_report_result(await self._make_json_request(build_ip_report_params(ip)))
# --- END GENERATED ---
