...
class TestDomainIQClientIntegration:
    ...
    def test_whois_lookup_domain(self) -> None:
        client = self.client
        assert client is not None
        result = client.whois_lookup(domain=TEST_DOMAINS[0])
    ...
    def test_whois_lookup_ip(self) -> None:
        client = self.client
        assert client is not None
        result = client.whois_lookup(ip=TEST_IP)
    ...
    def test_dns_lookup_basic(self) -> None:
        client = self.client
        assert client is not None
        result = client.dns_lookup(TEST_DOMAINS[0])
    ...
    def test_dns_lookup_with_types(self) -> None:
        client = self.client
        assert client is not None
        result = client.dns_lookup(
            TEST_DOMAINS[0],
    ...
    def test_domain_categorize(self) -> None:
        client = self.client
        assert client is not None
        result = client.domain_categorize([TEST_DOMAINS[0]])
    ...
    def test_domain_report(self) -> None:
        client = self.client
        assert client is not None
        result = client.domain_report(TEST_DOMAINS[0])
    ...
    def test_bulk_dns_lookup(self) -> None:
        client = self.client
        assert client is not None
        result = client.bulk_dns(TEST_DOMAINS[:2])
    ...
    def test_domain_search(self) -> None:
        client = self.client
        assert client is not None
    ...
    def test_email_report(self) -> None:
        client = self.client
        assert client is not None
        result = client.email_report(TEST_EMAIL)
    ...
    def test_reverse_search(self) -> None:
        client = self.client
        assert client is not None
        result = client.reverse_search("email", TEST_EMAIL)

        if result is None:
            pytest.skip("API returned None for reverse search")
        assert isinstance(result, dict)

    def test_monitor_list(self) -> None:
        client = self.client
        assert client is not None
        try:
    ...
    def test_error_handling_invalid_domain(self) -> None:
        ...
        assert client is not None
        try:
            result = client.whois_lookup(domain="invalid..domain..name")
    ...
    def test_client_session_reuse(self) -> None:
        client = self.client
        assert client is not None
        result1 = client.whois_lookup(domain=TEST_DOMAINS[0])
        result2 = client.dns_lookup(TEST_DOMAINS[0])
